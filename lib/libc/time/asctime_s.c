...
errno_t asctime_s(char *s, rsize_t maxsize, const struct tm *timeptr) {
  if (s == NULL || timeptr == NULL || maxsize < 26 || maxsize > RSIZE_MAX ||
      timeptr->tm_year < -1900 ||
      timeptr->tm_year > 8099 /* TODO: || not normalized */) {
  ...
  sprintf(s, "%s %s %2d %.2d:%.2d:%.2d %4d\n",
          ...
          _PDCLIB_lc_time_C.month_name_abbr[timeptr->tm_mon], timeptr->tm_mday,
