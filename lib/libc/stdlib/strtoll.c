...
long long int strtoll(const char *s, char **endptr, int base) {
  ...
  if (sign == '+') {
    rc = (long long int)_PDCLIB_strtox_main(
        &p, (unsigned)base, (uintmax_t)LLONG_MAX, (uintmax_t)(LLONG_MAX / base),
        (int)(LLONG_MAX % base), &sign);
  } else {
    rc = (long long int)_PDCLIB_strtox_main(
        &p, (unsigned)base, (uintmax_t)LLONG_MIN,
        (uintmax_t)(LLONG_MIN / -base), (int)(-(LLONG_MIN % base)), &sign);
  }
...
int main(void) {
  ...
  TESTCASE(errno == ERANGE);
  errno = 0;
  TESTCASE(strtoll("-9223372036854775807", NULL, 0) ==
  ...
  /* testing "even" overflow, i.e. base is power of two */
  TESTCASE(strtoll("170141183460469231731687303715884105728", NULL, 0) ==
           0x7fffffffffffffffffffffffffffffff);
  TESTCASE(errno == 0);
  TESTCASE(strtoll("170141183460469231731687303715884105729", NULL, 0) ==
           LLONG_MAX);
  TESTCASE(errno == ERANGE);
  errno = 0;
  TESTCASE(strtoll("-170141183460469231731687303715884105728", NULL, 0) ==
  ...
  TESTCASE(strtoll("-170141183460469231731687303715884105729", NULL, 0) ==
           LLONG_MIN);
  TESTCASE(errno == 0);
  TESTCASE(strtoll("-170141183460469231731687303715884105730", NULL, 0) ==
           LLONG_MIN);
  TESTCASE(errno == ERANGE);
  /* TODO: test "odd" overflow, i.e. base is not power of two */
#else
