...
*/
_PDCLIB_LOCAL _PDCLIB_fd_t _PDCLIB_open(const char *const filename,
                                        unsigned int mode);
...
/* A system call that repositions within a file. Returns new offset on success,
   -1 / errno on error.
*/
_PDCLIB_LOCAL _PDCLIB_int_least64_t _PDCLIB_seek(struct _PDCLIB_file_t *stream,
...
/* A system call that changes the mode of a given stream to that passed as
   ...
   *without* closing the stream. See comments in example implementation
   ...
   success.
*/
_PDCLIB_LOCAL int _PDCLIB_changemode(struct _PDCLIB_file_t *stream,
                                     unsigned int mode);
