...
static char colors[] = {
    [VTCOLOR_BLACK] = VGACOLOR_BLACK, [VTCOLOR_RED] = VGACOLOR_RED,
    [VTCOLOR_GREEN] = VGACOLOR_GREEN, [VTCOLOR_YELLOW] = VGACOLOR_BROWN,
    [VTCOLOR_BLUE] = VGACOLOR_BLUE,   [VTCOLOR_MAGENTA] = VGACOLOR_MAGENTA,
    [VTCOLOR_CYAN] = VGACOLOR_CYAN,   [VTCOLOR_GREY] = VGACOLOR_LIGHT_GRAY,
};
...
void paint_callback(vtconsole_t *vtc, vtcell_t *cell, int x, int y) {
  if (cell->attr.bright) {
    vga_cell(
        x, y,
        VGA_ENTRY(cell->c, brightcolors[cell->attr.fg], colors[cell->attr.bg]));
  } else {
    vga_cell(x, y,
